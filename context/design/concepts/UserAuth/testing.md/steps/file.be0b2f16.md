...
content_id: be0b2f161876834ee988aeaa2ec67ed1ca32744be7441ee526f3534dabd4ab49
---
...
Deno.test("UserAuthConcept", async (t) => {
  ...
  await t.step("Queries: _getUserDetails and _isModerator", async (t) => {
    ...
    await t.step("should correctly report moderator status", async () => {
      ...
      console.log(
        `Trace: Querying moderator status for ${frankId} (is a mod)...`,
      );
  ...
  await t.step(
    ...
    async () => {
      ...
      const initialAdminUser = await concept.users.findOne({
        username: "frank",
      });
      if (!initialAdminUser) {
        throw new Error(
          "Pre-existing admin user 'frank' not found for principle test setup.",
        );
      }
      const initialAdminId = initialAdminUser._id;
      const initialAdminStatus = await concept._isModerator({
        userId: initialAdminId,
      });
      assertEquals(
        (initialAdminStatus as any)[0].isModerator,
        true,
        "Setup: Initial admin 'frank' is a moderator.",
      );
      console.log(
        `Setup: Using pre-existing moderator '${initialAdminUser.username}' (${initialAdminId}) to grant privileges.`,
      );
