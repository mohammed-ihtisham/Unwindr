...
Deno.test("Principle: users must register and log in before contributing; moderators can verify content", async (t) => {
  ...
  try {
    ...
    await t.step("2. Register Admin User", async () => {
      ...
      console.log("  Output:", registerResult);
      assertEquals(isError(registerResult), false, "Admin registration should succeed");
    ...
    await t.step("4. Log in Admin User and grant moderator privilege to themselves", async () => {
      ...
      console.log(`  ✓ BobAdmin logged in with session: ${adminSessionToken}`);
      ...
      assertEquals(isError(grantSelfModResult), false, "Admin granting self-moderator should succeed");
      assertEquals(
      ...
      const adminIsModerator = await concept._isModerator({ userId: adminId });
    ...
    await t.step("5. User1 (non-admin) attempts to grant moderator status (should fail)", async () => {
      console.log(
      ...
      assertEquals(isError(grantResult), true, "Non-admin should not be able to grant moderator");
      assertEquals(
      ...
      console.log("  ✓ Alice correctly prevented from granting moderator status");
    });
    ...
    await t.step("6. Admin grants moderator privilege to User1", async () => {
      ...
      console.log("  Output:", grantResult);
      assertEquals(isError(grantResult), false, "Admin should be able to grant moderator");
      assertEquals(
    ...
    await t.step("7. User1 (now moderator) attempts to revoke moderator status (should succeed)", async () => {
      console.log(
      ...
      assertEquals(isError(revokeResult), false, "Alice (now moderator) should be able to revoke");
      assertEquals(
    ...
    await t.step("8. User1 logs out", async () => {
      ...
      const logoutResult = await concept.logout({ sessionToken: user1SessionToken });
...
Deno.test("Action: registerUser - ensures unique usernames, non-empty passwords", async (t) => {
  ...
  try {
    ...
    await t.step("Successful registration", async () => {
      ...
      assertEquals(user.canModerate, false, "Default canModerate should be false");
    ...
    await t.step("Rejection of empty password", async () => {
      ...
      console.log("  Output:", result);
      assertEquals(isError(result), true, "Empty password registration should fail");
      assertEquals(
        (result as { error: string }).error,
...
Deno.test("Action: login - handles correct/incorrect credentials", async (t) => {
  ...
  try {
    ...
    await t.step("Successful login with correct credentials", async () => {
      ...
      assertEquals(session.userId, userId, "Session should be linked to correct user");
    ...
    await t.step("Rejection with incorrect password", async () => {
      ...
      console.log("  Output:", loginResult);
      assertEquals(isError(loginResult), true, "Login with wrong password should fail");
      assertEquals(
    ...
    await t.step("Rejection with non-existent username", async () => {
      ...
      console.log("  Output:", loginResult);
      assertEquals(isError(loginResult), true, "Login with non-existent username should fail");
      assertEquals(
        (loginResult as { error: string }).error,
        "Invalid username or password",
        "Error message should indicate invalid credentials",
...
Deno.test("Action: logout - invalidates sessions", async (t) => {
  ...
  try {
    ...
    console.log(`[SETUP] Logged in user: ${username} with session: ${sessionToken}`);
    ...
    await t.step("Successful logout", async () => {
      ...
      const activeSession = await concept.activeSessions.findOne({ _id: sessionToken });
      assertEquals(activeSession, null, "Session should be removed from activeSessions");
      console.log("  ✓ Session removed from DB.");
    });

    await t.step("Attempt to logout with an already invalid session", async () => {
      console.log(
    ...
    await t.step("Attempt to logout with a non-existent session", async () => {
      ...
      console.log("  ✓ Attempt to logout non-existent session handled correctly.");
    });
  } finally {
    await client.close();
  }
...
Deno.test("Action: getAuthenticatedUser - retrieves user profile or null", async (t) => {
  ...
  try {
    ...
    await t.step("Return null for an invalid session token", async () => {
      ...
      console.log("  Output:", authResult);
      assertEquals(authResult.userProfile, null, "Should return null for invalid session");
      console.log("  ✓ Correctly returned null for invalid session.");
    });

    await t.step("Return null and clean up session if user is missing but session exists (data inconsistency)", async () => {
      ...
      console.log("  Output:", authResult);
      assertEquals(authResult.userProfile, null, "Should return null for ghost session");
      console.log("  ✓ Correctly returned null for ghost session.");

      // Verify the ghost session was cleaned up
      const ghostSession = await concept.activeSessions.findOne({ _id: ghostSessionToken });
      assertEquals(ghostSession, null, "Ghost session should have been deleted");
...
Deno.test("Action: changePassword - updates password and invalidates sessions", async (t) => {
  ...
  try {
    ...
    await t.step("Successful password change invalidates all sessions", async () => {
      console.log(
      ...
      assertEquals(isError(changeResult), false, "Password change should succeed");
      assertEquals(
      ...
      const auth1 = await concept.getAuthenticatedUser({ sessionToken: sessionToken1 });
      assertEquals(auth1.userProfile, null, "Session 1 should be invalidated");
      const auth2 = await concept.getAuthenticatedUser({ sessionToken: sessionToken2 });
      assertEquals(auth2.userProfile, null, "Session 2 should be invalidated");
    ...
    await t.step("Rejection with invalid old password", async () => {
      ...
      const reloginResult = await concept.login({ username, password: newPassword });
      const currentSession = (reloginResult as { sessionToken: ID }).sessionToken;
      ...
      console.log("  Output:", changeResult);
      assertEquals(isError(changeResult), true, "Password change with wrong old password should fail");
      assertEquals(
    ...
    await t.step("Rejection with invalid session token", async () => {
      ...
      console.log("  Output:", changeResult);
      assertEquals(isError(changeResult), true, "Password change with invalid session should fail");
      assertEquals(
        (changeResult as { error: string }).error,
        "Invalid session",
    ...
    await t.step("Rejection with empty new password", async () => {
      ...
      console.log("  Output:", changeResult);
      assertEquals(isError(changeResult), true, "Password change with empty new password should fail");
      assertEquals(
        (changeResult as { error: string }).error,
...
Deno.test("Action: grantModerator - assigns moderator privileges", async (t) => {
  ...
  try {
    ...
    regularUserSessionToken = (regularLoginResult as { sessionToken: ID }).sessionToken;

    console.log(`[SETUP] Admin user "${adminUser}" (ID: ${adminId}) is a moderator.`);
    console.log(
    ...
    await t.step("Successful grant by admin to regular user", async () => {
      ...
      console.log("  Output:", grantResult);
      assertEquals(isError(grantResult), false, "Grant moderator should succeed");
      assertEquals(
        (grantResult as { success: boolean }).success,
        true,
    ...
    await t.step("Idempotency: Granting moderator twice to same user", async () => {
      console.log(
      ...
      console.log("  Output:", grantResult);
      assertEquals(isError(grantResult), false, "Grant moderator (idempotent) should succeed");
      assertEquals(
        (grantResult as { success: boolean }).success,
        true,
    ...
    await t.step("Rejection: Non-admin user attempts to grant moderator", async () => {
      console.log(
        `\n[ACTION] "${regularUser}" (now moderator) attempts to grant moderator to "${otherUser}"`,
      );
      const grantResult = await concept.grantModerator({
      ...
      assertEquals(
        isError(grantResult),
        true,
        "User not setup as an admin should not be able to grant moderator",
      );
      assertEquals(
        (grantResult as { error: string }).error,
        "Admin user does not have moderator privileges",
      ...
      assertEquals(
        ...
        false,
        "Other user should still not be a moderator",
      );
      console.log(`  ✓ Non-admin "${regularUser}" correctly prevented.`);
    });
    ...
    await t.step("Rejection: Invalid admin session token", async () => {
      ...
      console.log("  Output:", grantResult);
      assertEquals(isError(grantResult), true, "Grant with invalid session should fail");
      assertEquals(
        (grantResult as { error: string }).error,
    ...
    await t.step("Rejection: Non-existent target user", async () => {
      ...
      console.log("  Output:", grantResult);
      assertEquals(isError(grantResult), true, "Grant to non-existent user should fail");
      assertEquals(
        (grantResult as { error: string }).error,
...
Deno.test("Action: revokeModerator - removes moderator privileges", async (t) => {
  ...
  try {
    ...
    await concept.grantModerator({ targetUserId: modUserId, adminSessionToken });
    ...
    console.log(`[SETUP] Admin user "${adminUser}" (ID: ${adminId}) is a moderator.`);
    console.log(`[SETUP] Moderator user "${modUser}" (ID: ${modUserId}) is a moderator.`);
    console.log(
    ...
    await t.step("Successful revoke by admin of a moderator", async () => {
      ...
      console.log("  Output:", revokeResult);
      assertEquals(isError(revokeResult), false, "Revoke moderator should succeed");
      assertEquals(
        (revokeResult as { success: boolean }).success,
        true,
    ...
    await t.step("Idempotency: Revoking moderator twice from same user", async () => {
      console.log(
      ...
      console.log("  Output:", revokeResult);
      assertEquals(isError(revokeResult), false, "Revoke moderator (idempotent) should succeed");
      assertEquals(
        (revokeResult as { success: boolean }).success,
        true,
    ...
    await t.step("Rejection: Non-admin user attempts to revoke moderator", async () => {
      // Re-grant mod status to modUser for this test
      await concept.grantModerator({ targetUserId: modUserId, adminSessionToken });
      const currentModStatus = await concept._isModerator({ userId: modUserId });
      assertEquals(
      ...
      console.log(
        `\n[ACTION] "${modUser}" (moderator but not admin) attempts to revoke "${adminUser}"`,
      );
      const revokeResult = await concept.revokeModerator({
        targetUserId: adminId,
      ...
      assertEquals(
        ...
        true,
        "Moderator not setup as an admin should not be able to revoke moderator",
      );
      assertEquals(
      ...
      assertEquals(
        ...
        true,
        "Admin should still be a moderator",
      );
      console.log(`  ✓ Non-admin moderator "${modUser}" correctly prevented.`);
    });
    ...
    await t.step("Rejection: Invalid admin session token", async () => {
      ...
      console.log("  Output:", revokeResult);
      assertEquals(isError(revokeResult), true, "Revoke with invalid session should fail");
      assertEquals(
        (revokeResult as { error: string }).error,
        "Invalid admin session",
        "Error message should indicate invalid admin session",
    ...
    await t.step("Rejection: Non-existent target user", async () => {
      ...
      console.log("  Output:", revokeResult);
      assertEquals(isError(revokeResult), true, "Revoke from non-existent user should fail");
      assertEquals(
        (revokeResult as { error: string }).error,
        "Target user not found",
        "Error message should indicate target user not found",
...
Deno.test("Query: _getUserDetails - retrieves user profile", async (t) => {
  ...
  try {
    ...
    await t.step("Retrieve details for an existing user", async () => {
      ...
      assertEquals(detailsResult.length, 1, "Result array should contain one item");
      assertEquals(detailsResult[0].user.username, username, "Username should match");
      assertEquals(
    ...
    await t.step("Return error for a non-existent user", async () => {
      ...
      console.log("  Output:", detailsResult);
      assertEquals(isError(detailsResult), true, "Query for non-existent user should return error");
      assertEquals(
...
Deno.test("Query: _isModerator - checks moderator status", async (t) => {
  ...
  try {
    ...
    await concept.grantModerator({ targetUserId: moderatorUserId, adminSessionToken });

    console.log(`[SETUP] Regular user "${regularUser}" (ID: ${regularUserId})`);
    console.log(`[SETUP] Moderator user "${moderatorUser}" (ID: ${moderatorUserId})`);

    await t.step("Check moderator status for a regular user (should be false)", async () => {
      console.log(`\n[QUERY] _isModerator({ userId: "${regularUserId}" })`);
      const isModResult = await concept._isModerator({ userId: regularUserId });
      ...
      assertEquals(isModResult.length, 1, "Result array should contain one item");
      assertEquals(
        (isModResult as { isModerator: boolean }[])[0].isModerator,
        false,
        "Regular user should not be a moderator",
      );
      console.log("  ✓ Correctly identified regular user as not a moderator.");
    });

    await t.step("Check moderator status for a moderator user (should be true)", async () => {
      console.log(`\n[QUERY] _isModerator({ userId: "${moderatorUserId}" })`);
      const isModResult = await concept._isModerator({ userId: moderatorUserId });
      console.log("  Output:", isModResult);
      assertEquals(isError(isModResult), false, "Query should succeed");
      assert(Array.isArray(isModResult), "Result should be an array");
      assertEquals(isModResult.length, 1, "Result array should contain one item");
    ...
    await t.step("Return error for a non-existent user", async () => {
      ...
      console.log("  Output:", isModResult);
      assertEquals(isError(isModResult), true, "Query for non-existent user should return error");
      assertEquals(
