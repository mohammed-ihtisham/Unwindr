...
Deno.test("Principle: QualityRanking promotes well-loved places regardless of popularity, helping users discover authentic local favorites", async () => {
  ...
  try {
    ...
    const scoreA_res = await concept.calculateQualityScore({
      placeId: placeA_id,
    });
    const scoreB_res = await concept.calculateQualityScore({
      placeId: placeB_id,
    });
    const scoreC_res = await concept.calculateQualityScore({
      placeId: placeC_id,
    });
    const scoreD_res = await concept.calculateQualityScore({
      placeId: placeD_id,
    });
    ...
    assertEquals(
      isError(scoreA_res),
      false,
      "Score A calculation should succeed.",
    );
    assertEquals(
      isError(scoreB_res),
      false,
      "Score B calculation should succeed.",
    );
    assertEquals(
      isError(scoreC_res),
      false,
      "Score C calculation should succeed.",
    );
    assertEquals(
      isError(scoreD_res),
      false,
      "Score D calculation should succeed.",
    );
    ...
    console.log(
      `  Place A Quality Score: ${scoreA} (Engagement: 400, Visits: 500)`,
    );
    console.log(
      `  Place B Quality Score: ${scoreB} (Engagement: 100, Visits: 20)`,
    );
    console.log(
      `  Place C Quality Score: ${scoreC} (Engagement: 5, Visits: 10)`,
    );
    console.log(
      `  Place D Quality Score: ${scoreD} (Engagement: 100, Visits: 100)`,
    );
    ...
    console.log(
      `  ✓ Standard user recommendations match: ${
        JSON.stringify(standardRecs)
      }`,
    );
    ...
    console.log(
      `  ✓ Emergent user recommendations match: ${
        JSON.stringify(emergentRecs)
      }`,
    );
...
Deno.test("Action: updateMetrics successfully updates metrics and enforces requirements", async () => {
  ...
  try {
    ...
    console.log(
      `  Input: { placeId: ${place1_id}, visits: 10, engagement: 5 }`,
    );
    ...
    const metrics1 = await concept["rankingMetrics"].findOne({
      _id: place1_id,
    });
    assertEquals(
    ...
    console.log(
      `  Input: { placeId: ${place1_id}, visits: 20, engagement: 15 }`,
    );
    ...
    const metrics2 = await concept["rankingMetrics"].findOne({
      _id: place1_id,
    });
    assertEquals(
    ...
    const metrics3 = await concept["rankingMetrics"].findOne({
      _id: place2_id,
    });
    assertEquals(
...
Deno.test("Action: calculateQualityScore computes and updates scores correctly", async () => {
  ...
  try {
    ...
    await concept.updateMetrics({
      ...
      visits: 0,
      engagement: 10,
    }); // Expected Score = 10.0 (10 / max(0,1))
    ...
    const metricsA = await concept["rankingMetrics"].findOne({
      _id: placeA_id,
    });
    assertEquals(
    ...
    const metricsB = await concept["rankingMetrics"].findOne({
      _id: placeB_id,
    });
    assertEquals(
    ...
    const metricsC = await concept["rankingMetrics"].findOne({
      _id: placeC_id,
    });
    assertEquals(
...
Deno.test("Action: setPreferences stores and updates user preferences and enforces requirements", async () => {
  ...
  try {
    ...
    const prefs1 = await concept["rankingPreferences"].findOne({
      _id: user1_id,
    });
    assertEquals(
    ...
    const prefs2 = await concept["rankingPreferences"].findOne({
      _id: user1_id,
    });
    assertEquals(
...
Deno.test("Query: _getRecommendedPlaces retrieves and ranks places according to user preferences", async () => {
  ...
  try {
    ...
    console.log(
      `  ✓ Emergent user recommendations correct: ${
        JSON.stringify(emergentRecs)
      }`,
    );
