...
Deno.test("Principle: Users can add, set verification status, and update places; system can seed from provider", async () => {
  ...
  try {
    ...
    // Step 1: System can seed from provider (using bulk import for faster testing)
    console.log(
    ...
    console.log(`  Input: { osmDataPath: "${sampleFilePath}" }`);
    const seedResult = await placeCatalog.bulkImportOSMPlaces({
    ...
    const { coordinates: [lng, lat] } = initialSeededPlaces[0].location;
    const centerLat = lat;
    const centerLng = lng;
...
Deno.test("Action: bulkImportOSMPlaces successfully imports from GeoJSON file", async () => {
  console.log("\n=== Testing bulkImportOSMPlaces Action ===");
  const [db, client] = await testDb();
  const placeCatalog = new PlaceCatalogConcept(db);

  try {
    // Should successfully import places from GeoJSON file
    ...
    assertEquals(isError(result), false, "Bulk import should succeed.");
    console.log("  ✓ Bulk import completed without errors");

    const importedCount = await placeCatalog.places.countDocuments();
    console.log(`  ✓ ${importedCount} places imported`);
    // Sample file has 7 features, but 1 has no name so should be skipped
    assertEquals(
      importedCount,
      6,
      "Should import 6 places (7 in file, 1 without name skipped).",
    );

    console.log("\n[VERIFY] Check imported place details");
    const coffeeShop = await placeCatalog.places.findOne({
      name: "Sample Coffee Shop",
    });
    assertExists(coffeeShop, "Coffee shop should be imported.");
    assertEquals(coffeeShop?.category, "Cafe");
    assertEquals(coffeeShop?.source, "provider");
    assertEquals(coffeeShop?.verified, true);
    assertEquals(coffeeShop?.addedBy, "system:osm");
    assertEquals(
      coffeeShop?.address,
      "123, Main Street, Boston, MA, 02108",
    );
    console.log("  ✓ Coffee shop imported with correct details");

    const park = await placeCatalog.places.findOne({ name: "Boston Common" });
    assertExists(park, "Park should be imported.");
    assertEquals(park?.category, "Park");
    console.log("  ✓ Park imported with correct category");

    const restaurant = await placeCatalog.places.findOne({
      name: "Sample Restaurant",
    });
    assertExists(restaurant, "Restaurant should be imported.");
    assertEquals(restaurant?.category, "Restaurant");
    console.log("  ✓ Restaurant imported with correct category");

    // Should fail with invalid file path
    console.log("\n[TEST] Reject bulk import with invalid file path");
    console.log('  Input: { osmDataPath: "nonexistent.geojson" }');
    const resultInvalidPath = await placeCatalog.bulkImportOSMPlaces({
      osmDataPath: "nonexistent.geojson",
    });
    console.log("  Output:", resultInvalidPath);
    assertEquals(
      isError(resultInvalidPath),
      true,
      "Should return error for invalid file path.",
    );
    console.log("  ✓ Correctly rejected invalid file path");

    // Should fail with invalid GeoJSON format
    console.log("\n[TEST] Reject bulk import with invalid GeoJSON format");
    // Create a temporary invalid JSON file
    const invalidFilePath = "./src/concepts/PlaceCatalog/invalid-test.json";
    await Deno.writeTextFile(invalidFilePath, '{"type": "FeatureCollection"}');
    console.log(`  Input: { osmDataPath: "${invalidFilePath}" }`);
    const resultInvalidFormat = await placeCatalog.bulkImportOSMPlaces({
      osmDataPath: invalidFilePath,
    });
    console.log("  Output:", resultInvalidFormat);
    assertEquals(
      isError(resultInvalidFormat),
      true,
      "Should return error for invalid GeoJSON format.",
    );
    assertEquals(
      (resultInvalidFormat as { error: string }).error,
      "Invalid GeoJSON format: missing features array",
    );
    // Clean up
    await Deno.remove(invalidFilePath);
    console.log("  ✓ Correctly rejected invalid GeoJSON format");
...
Deno.test("Action: seedPlaces successfully seeds places and enforces requirements", async () => {
  ...
  try {
    // Should successfully seed places from Overpass API
    console.log("\n[TEST] Seed places from Overpass API successfully");
    const initialCount = await placeCatalog.places.countDocuments();
    assertEquals(initialCount, 0, "Collection should be empty initially.");
    console.log("  ✓ Collection is empty initially");

    // Use the seedPlaces method (seeds Cambridge and Boston)
    console.log("  Input: {} (seeds Cambridge and Boston areas)");
    ...
    console.log(`  ✓ ${seededCount} places seeded from Overpass API`);
    // Note: We can't guarantee a specific count since it depends on OSM data

    // Should handle duplicate prevention on second seed
    ...
    console.log(
      `  ✓ Collection count: ${finalCount} (duplicates were prevented)`,
    );

    // Should skip seeding if database already has places
    console.log("\n[TEST] Skip seeding when database already has places");
    console.log("  Input: {} (seeds Cambridge and Boston areas)");
    const resultSkipSeeding = await placeCatalog.seedPlaces();
    console.log("  Output:", resultSkipSeeding);
    assertEquals(
      isError(resultSkipSeeding),
      false,
      "Should not error when skipping seeding.",
    );
    console.log("  ✓ Correctly skipped seeding when database has places");

    // Should skip seeding if database already has places
    console.log("\n[TEST] Skip seeding when database already has places");
    console.log("  Input: {} (seeds Cambridge and Boston areas)");
    const resultSkipSeeding2 = await placeCatalog.seedPlaces();
    console.log("  Output:", resultSkipSeeding2);
    assertEquals(
      isError(resultSkipSeeding2),
      false,
      "Should not error when skipping seeding.",
    );
    console.log("  ✓ Correctly skipped seeding when database has places");
...
Deno.test("Query: _getPlacesInArea retrieves places within specified area and enforces requirements", async () => {
  ...
  try {
    ...
    // Setup: Seed places using bulk import
